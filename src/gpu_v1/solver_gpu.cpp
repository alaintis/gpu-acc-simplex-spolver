...
solver(int m, int n_total, const mat_csc& A, const vec& b, const vec& c, vec& x, const idx& B_init) {
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        }
        {
            ...
            // Run reduction kernel & return status directly via mapped memory
            gpu_pricing_dantzig(n);
        }

        if (gpu_peek_optimality() && gpu_check_optimality()) {
        ...
        // 8. Update Basis
        {
            ...
            gpu_update_state_fused(m);
            gpu_update_basis_fast(m);
        }
    }
