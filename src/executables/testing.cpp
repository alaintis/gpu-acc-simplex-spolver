...
bool run_solver_test(const Problem& p, const std::string& problem_name) {
    ...
    std::vector<double> backend_timings;
    ...
    for(int i=0; i<10; i++){
        try {
            ...
            auto end = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double, std::milli> ms = end - t;
            backend_timings.push_back(ms.count());
    ...
    // Compare results
        ...
        double total_backend_time = std::accumulate(backend_timings.begin(), backend_timings.end(), 0.0);
        double avg_backend_time = total_backend_time / backend_timings.size();
        double min_backend_time = *std::min_element(backend_timings.begin(), backend_timings.end());
        double max_backend_time = *std::max_element(backend_timings.begin(), backend_timings.end());
        ...
        std::cout << "" << score_backend << std::endl;
        std::cout << "" << total_backend_time << "," << avg_backend_time << "," << min_backend_time << "," << max_backend_time << std::endl;
