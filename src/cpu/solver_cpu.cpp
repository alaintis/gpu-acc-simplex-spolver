...
solver(int m, int n_total, const mat& A, const vec& b, const vec& c, vec& x, const idx& B_init) {
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        for (int i = 0; i < m; i++) {
            int col_idx = B[i];
            const double* col_ptr = &A_scaled[col_idx * m];
            for (int k = 0; k < m; k++)
                A_B[k + i * m] = col_ptr[k];
            c_B[i] = c_scaled[col_idx];
        }

        // 2. Factorize Basis
        cpu_factorize(m, A_B.data());
        ...
        const vector<double>& b_curr = is_perturbed ? b_eff : b_scaled;
        cpu_solve_prefactored(m, b_curr.data(), x_B.data(), false);
        ...
        if (stall_counter > 30 && !is_perturbed) {
            ...
            cpu_solve_prefactored(m, b_eff.data(), x_B.data(), false);
            for (int i = 0; i < m; ++i) {
                x_scaled[B[i]] = x_B[i];
            }
        }
        ...
        cpu_solve_prefactored(m, c_B.data(), y.data(), true);

        // 5. Pricing (Dantzig's Rule for now)
        int j_i = -1;
        double min_sn = -1e-7; // Tolerance
        bool optimal = true;

        for (int i = 0; i < n; i++) {
            int original_idx = N[i];
            double dot_prod = 0.0;
            const double* col_ptr = &A_scaled[original_idx * m];
            for (int k = 0; k < m; k++)
                dot_prod += col_ptr[k] * y[k];

            double sn = c_scaled[original_idx] - dot_prod;
